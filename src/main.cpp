...
#if defined(NDEBUG)
# error "Olivecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x2591f837111cb01b87c221d9f9c45b4fea9966ce5942d52fe1685930fa5781c6");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Olivecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Olivecoin Signed Message:\n";
...
{
    // Olivecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Olivecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Olivecoin: 3 days
static const int64 nTargetSpacing = 2 * 60; // Olivecoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Olivecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Olivecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x52c865d07f6a0cc20ed0a03d4911ec5ab01ac4b911d1475aab6785b6fc3737a3");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "02/23/2021 - Creation date of Olivecoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0494133a518c5466e35a779d1ee59f7e2b15f6e18efa5887d363ec39112c006947252b44bd0b8698b79de7e2fdce826ed0d86f295ac18824dcedfe9d84385a8350") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1614079479;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085985767;

        if (fTestNet)
        {
            block.nTime    = 1614079437;
            block.nNonce   = 386953002;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xd32c56e31513d7a834a79a5d478d339bf94c4370be82bcfb55b08dd463474146"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc2, 0xb4, 0xdc }; // Olivecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("OlivecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("OlivecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static OlivecoinMiner(CWallet *pwallet)
{
    printf("OlivecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("olivecoin-miner");
    ...
    try { loop {
        ...
        printf("Running OlivecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("OlivecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&OlivecoinMiner, pwallet));
}
